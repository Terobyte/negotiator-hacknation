...
_PRICE_SHAPED_MIN = Decimal(100)  # bare/word amounts at/above this look like a price, not a duration/count
...
class HonestyGate:
    ...
    def _unsupported_claim_reason(self, text: str, facts: tuple[LedgerFact, ...]) -> str | None:
        ...
        bare_amounts = _bare_amounts(text)
        bare_word_amounts = set(map(Decimal, _bare_word_amounts(text)))
        if quote_language:
            amounts.update(bare_amounts)
            amounts.update(bare_word_amounts)
        else:
            # Quote-language is a trigger for checking ALL bare amounts, not a precondition
            # for checking any of them -- a price-shaped bare number (>= _PRICE_SHAPED_MIN)
            # must be backed by a ledger fact even with no "$"/"quote"/"price" cue (BUG-55).
            amounts.update(amount for amount in bare_amounts if amount >= _PRICE_SHAPED_MIN)
            amounts.update(amount for amount in bare_word_amounts if amount >= _PRICE_SHAPED_MIN)
        supported_amounts = {
