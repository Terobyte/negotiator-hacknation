...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${url}/api/whisper`, {method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json",Origin:new URL(request.url).origin},body:await request.text()});
